...
const matchDetailsDBObjectToResponseObject = (dbObject) => {
  return {
...
app.get("/players/", async (request, response) => {
  ...
  response.send(
    playersArray.map((eachPlayer) => playerDBObjectToResponseObject(eachPlayer))
  );
});
...
app.get("/matches/:matchId/", async (request, response) => {
  ...
  response.send(matchDetailsDBObjectToResponseObject(matchDetails));
});
...
app.get("/players/:playerId/matches/", async (request, response) => {
  ...
  response.send(
    playerMatches.map((eachMatch) =>
      matchDetailsDBObjectToResponseObject(eachMatch)
    )
  );
});
